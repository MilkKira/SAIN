...
public class Enemy : BotBase, ISPlayer
{
    ...
    private float LookUpdateInterval()
    {
        ...
        return interval * Random.Range(0.75f, 1.25f);
    }
        ...
        : base(bot)
    {
        ...
        _nextCheckLookTime = Time.time + Random.Range(0, 1f);
    }
    ...
    // 待发送的报告数据
    private float _nextSquadReportTime = -1f;
    private Vector3 _pendingReportPosition;
    private bool _pendingReportIsSeen;

    /// <summary>
    /// 在 Tick 中调用，检查是否到了延迟发送的时间
    /// </summary>
    private void CheckSquadReport(float currentTime)
    {
        if (_nextSquadReportTime > 0 && currentTime >= _nextSquadReportTime)
        {
            // 仅处理目视报告（我们的 ScheduleSquadReport 调用全部传入 isSeen=true）
            if (_pendingReportIsSeen)
            {
                EnemyPlace place = KnownPlaces.UpdateSeenPlace(_pendingReportPosition, currentTime);
                if (place != null)
                {
                    // 施加坐标误差
                    Vector3 errorPosition = ApplyReportError(place.Position);
                    // 用误差坐标重新获取一个新的 EnemyPlace（确保位置被正确记录）
                    EnemyPlace errorPlace = KnownPlaces.UpdateSeenPlace(errorPosition, currentTime);
                    if (errorPlace != null)
                    {
                        Bot.Squad.SquadInfo?.ReportEnemyPosition(this, errorPlace, true, currentTime);
                    }
                }
            }
        }
        // 重置计时器
        _nextSquadReportTime = -1f;
    }

    /// <summary>
    /// 给坐标添加随机误差（平面 + 垂直）
    /// </summary>
    private Vector3 ApplyReportError(Vector3 position)
    {
        float baseError = Mathf.Clamp(RealDistance * 0.05f, 0.5f, 5f); // 距离的5%，最小0.5米，最大5米
        float offsetX = Random.Range(-baseError, baseError);
        float offsetZ = Random.Range(-baseError, baseError);
        float offsetY = Random.Range(-baseError * 0.3f, baseError * 0.3f); // 垂直误差小一些
        return position + new Vector3(offsetX, offsetY, offsetZ);
    }
    ...
    public void UpdateLastSeenPosition(Vector3 position, float currentTime)
    {
        ...
        // 不再直接报告，而是设定延迟报告
        ScheduleSquadReport(position, true, currentTime);
    }
    ...
    /// <summary>
    /// 安排一次延迟报告。如果已有待发送报告，则更新位置但不重置延迟时间（保留最早的计时器）。
    /// 你也可以选择每次都重置，这里采用“保留最早计时器”以体现通信延迟的一致性。
    /// </summary>
    private void ScheduleSquadReport(Vector3 position, bool isSeen, float currentTime)
    {
        // 如果还没有安排报告，则设定一个新的延迟
        if (_nextSquadReportTime <= 0)
        {
            float delay = Random.Range(0.5f, 2.0f);
            _nextSquadReportTime = currentTime + delay;
        }

        _pendingReportPosition = position;
        _pendingReportIsSeen = isSeen;
    }

    // 原有的报告接收不需要改（队友转述误差由别处控制，或者我们也可以在此加误差，但保持原样）
    ...
    public void SetEnemyAsSniper(bool isSniper)
    {
        ...
        {
            Bot.Talk.TalkAfterDelay(EPhraseTrigger.SniperPhrase, ETagStatus.Combat, Random.Range(0.33f, 0.66f));
        }
    }
