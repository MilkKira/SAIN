...
{
    // 误判概率：15% 的几率忽略观察结果
    private const float ERROR_CHANCE = 0.15f;
    ...
    public void TickEnemy(float currentTime)
    {
        if (Enemy.EnemyKnown)
        {
            ...
            if (_nextCheckEnemyLookTime < currentTime)
            {
                ...
                // 是否在看我：15% 概率忽略
                bool looksAtMe = Vector3.Dot(
                    (Bot.Position - EnemyTransform.WeaponRoot).normalized,
                    EnemyTransform.LookDirection
                ) >= DOT_THRESHOLD_LOOK_AT_ME;

                if (looksAtMe && Random.value > ERROR_CHANCE) // 85% 概率报告
                {
                    EnemyLookAtMe = true;
                }
                // 若被忽略，EnemyLookAtMe 保持原有状态（可能已过期变为 false）

                // 是否用枪指着我：15% 概率忽略
                bool pointsAtMe = Enemy.IsShooter()
                    && Vector3.Dot(
                ...
                if (pointsAtMe && Random.value > ERROR_CHANCE)
                {
                    PointingWeaponAtMe = true;
                }
            }
    ...
    {
        EEnemyAction result = EEnemyAction.None;

        if (EnemyUsingSurgery)
            result = EEnemyAction.UsingSurgery;
        else if (EnemyIsReloading)
            result = EEnemyAction.Reloading;
        else if (EnemyHasGrenadeOut)
            result = EEnemyAction.HasGrenade;
        else if (EnemyIsHealing)
            result = EEnemyAction.Healing;
        else if (EnemyIsLooting)
            result = EEnemyAction.Looting;

        // 误判：如果检测到动作，15% 概率视而不见，返回 None
        if (result != EEnemyAction.None && Random.value < ERROR_CHANCE)
